...
void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
                     FILE** file, float** data, long* file_size) {
    ...
    *data = (float*)malloc(*file_size - sizeof(Config));
...
unsigned int random_u32(unsigned long *state) {
...
float random_f32(unsigned long *state) { /* random float32 in [0,1) */
...
int main(int argc, char *argv[]) {
    ...
    for (i = 2; i < argc; i += 2) {
        ...
        else if (argv[i][1] == 's') { rng_seed = (unsigned long)atoi(argv[i + 1]); }
    ...
    if (rng_seed <= 0) rng_seed = (unsigned long)time(NULL);
